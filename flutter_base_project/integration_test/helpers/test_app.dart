...
/// điều hành có thể treo test — nếu sau này flow nào thật sự cần bấm qua
/// dialog permission của OS, dùng native automation của Patrol,
/// `$.native`/`$.platform.mobile`, thay vì gọi `main()` thật). Ta chỉ cần
/// DI + `MyApp` để lái luồng UI.
///
...
///
/// Nhận [PatrolIntegrationTester] (`$`, tham số duy nhất của `patrolTest`) thay vì
/// `WidgetTester` thuần — `$.tester` cho lại đúng `WidgetTester` bên dưới,
/// dùng khi cần API pump/pumpAndSettle với duration cụ thể như dưới đây
/// (Page Object mới dùng `$(key)`/`.tap()`/`.enterText()` của Patrol để tìm
/// và thao tác widget, xem `pages/login_page_object.dart`).
...
/// `flutter drive` (xem `test_driver/integration_test.dart`) hoặc `patrol
/// test`: driver/patrol sẽ ghi các ảnh này ra
/// `integration_test/reports/<run>/screenshots/*.png` và gom vào
/// `report.html` sau khi test chạy xong.
///
/// Khi chạy bằng `flutter test` (không qua driver/patrol runner), lệnh vẫn
/// thực thi nhưng ảnh chụp không được driver nào lấy ra để ghi file — dùng
/// để debug tại chỗ là chính, muốn có report ảnh phải chạy qua `flutter
/// drive` hoặc `patrol test`.
///
