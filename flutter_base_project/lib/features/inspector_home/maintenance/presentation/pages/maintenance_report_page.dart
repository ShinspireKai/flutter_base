...
// ─────────────────────────────────────────────────────────────────────────────
// MaintenanceReportPage — BaseView (MVP View) cho「維修回報」(R2), lần đầu
// thợ sửa xử lý 1 nhiệm vụ (task.reworkRound == 0).
// Vòng lặp再維修 sau khi複檢不通過 dùng màn hình riêng: MaintenanceReworkPage (R3).
// ─────────────────────────────────────────────────────────────────────────────

@RoutePage()
class MaintenanceReportPage extends BaseView {
  final MaintenanceTaskEntity task;

  const MaintenanceReportPage({super.key, required this.task});

  @override
  State<MaintenanceReportPage> createState() => _MaintenanceReportPageState();
}

class _MaintenanceReportPageState
    extends BaseViewState<MaintenanceReportPresenter, MaintenanceReportPage>
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (task.reporterName != null) ...[
                          _buildOriginalReportBlock(task),
                        ],
                        const SizedBox(height: 8),
                        Row(
                          children: [
  ...
  String _titleFor(MaintenanceTaskEntity task) {
    ...
    return l10n.maintenanceReportTitle;
  }
  ...
  Widget _buildOriginalReportBlock(MaintenanceTaskEntity task) {
    final l10n = AppLocalizations.of(context)!;
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
          task.isExternalAssignment
              ? l10n.assignedByInline(
                  task.reporterName!,
                  _formatDateTime(task.reportedAt!),
                )
              : l10n.reportedByInline(
                  task.reporterName!,
                  _formatDateTime(task.reportedAt!),
                ),
          style: TextStyle(fontSize: 13, color: ColorName.colorTextGrey),
        ),
        if (task.reportDescription != null) ...[
          const SizedBox(height: 6),
          Text(
            task.reportDescription!,
            style: TextStyle(fontSize: 15, color: ColorName.redDanger),
          ),
        ],
        SizedBox(height: 8),
        if (task.reportPhotoPaths.isNotEmpty) ...[
          ...
          Container(
            ...
            child: Text(
              '📷 ${l10n.reportPhotosLabel(task.reportPhotoPaths.length)}',
              style: TextStyle(fontSize: 13, color: ColorName.greenPrimary),
            ),
          ),
        ],
      ],
  ...
  Widget _buildCompletionNoteField() {
    ...
    return CustomTextField(
      ...
      labelText: l10n.completionNoteLabel,
        ...
        SizedBox(
          ...
          child: ElevatedButton(
            ...
            child: isSubmitting
                ...
                : Text(
                    l10n.completeSendRecheckButton,
                    style: const TextStyle(
                      fontSize: 15,
                      fontWeight: FontWeight.w700,
                    ),
